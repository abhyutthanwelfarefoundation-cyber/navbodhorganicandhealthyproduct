...
const Checkout = () => {
  ...
  const [form, setForm]       = useState({ name: '', phone: '', email: '', address: '', city: 'Raipur', paymentMethod: 'upi', notes: '' });
  ...
  const handleDetails = (e) => {
    ...
    if (form.phone.length !== 10 || !/^[6-9]/.test(form.phone)) return toast.error('Enter a valid 10-digit mobile number');
  ...
  const handlePlace = async () => {
    ...
    try {
      ...
      navigate(`/order-success?id=${result.orderId || result._id || 'NVB-' + Date.now().toString().slice(-6)}&name=${encodeURIComponent(form.name)}&phone=${encodeURIComponent(form.phone)}&total=${total}&method=${form.paymentMethod}&txn=${encodeURIComponent(txnId)}`);
    } catch {
  ...
  if (step === 'payment') return (
    <div style={{ paddingTop: 'var(--nav-h)', background: 'var(--cream)', minHeight: '100vh' }}>
      <div style={{ background: 'var(--beige-warm)', borderBottom: '1px solid var(--border-warm)', padding: '28px 0' }}>
        <div className="container">
          <div className="eyebrow" style={{ marginBottom: 8 }}>💳 Payment</div>
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(24px,4vw,38px)', color: 'var(--ink)' }}>
            Complete Payment
          </h1>
        </div>
      </div>

      <div className="container" style={{ padding: 'clamp(24px,4vw,48px) 28px', maxWidth: 560 }}>

        {/* Amount banner */}
        <div style={{ background: 'var(--forest-deep)', borderRadius: 20, padding: '24px 28px', marginBottom: 24, textAlign: 'center' }}>
          <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 13, marginBottom: 6 }}>Amount to Pay</div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(40px,6vw,56px)', fontWeight: 700, color: 'var(--beige)' }}>₹{total}</div>
          <div style={{ color: 'rgba(255,255,255,0.6)', fontSize: 12, marginTop: 4 }}>To: {UPI_NAME}</div>
        </div>

        {/* QR Code */}
        <div style={{ background: 'white', borderRadius: 20, padding: 28, border: '1px solid var(--border)', boxShadow: 'var(--shadow-md)', textAlign: 'center', marginBottom: 20 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink-soft)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
            Scan QR to Pay
          </div>

          {/* QR Image */}
          <div style={{ display: 'inline-block', padding: 12, background: 'white', borderRadius: 16, border: '2px solid var(--forest-mist)', marginBottom: 16 }}>
            <img src={QR_URL} alt="UPI QR Code" width={200} height={200}
              style={{ display: 'block', borderRadius: 8 }}
              onError={e => { e.target.style.display = 'none'; }}
            />
          </div>

          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink)', marginBottom: 4 }}>UPI ID</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16 }}>
            <code style={{ background: 'var(--forest-mist)', padding: '6px 14px', borderRadius: 8, fontSize: 15, fontWeight: 700, color: 'var(--forest-deep)', letterSpacing: '0.04em' }}>
              {UPI_ID}
            </code>
            <button onClick={() => { navigator.clipboard.writeText(UPI_ID); toast.success('UPI ID copied!'); }}
              style={{ background: 'var(--forest)', color: 'white', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              Copy
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
            {[
              { name: 'GPay',     url: `gpay://upi/pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_NAME)}&am=${total}&cu=INR` },
              { name: 'PhonePe',  url: `phonepe://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_NAME)}&am=${total}&cu=INR` },
              { name: 'Paytm',    url: `paytmmp://pay?pa=${UPI_ID}&pn=${encodeURIComponent(UPI_NAME)}&am=${total}&cu=INR` },
            ].map(app => (
              <a key={app.name} href={app.url}
                style={{ background: 'var(--cream)', border: '1.5px solid var(--border)', borderRadius: 10, padding: '8px 16px', fontSize: 13, fontWeight: 700, color: 'var(--ink)', textDecoration: 'none', display: 'inline-block' }}>
                {app.name}
              </a>
            ))}
          </div>
          <div style={{ fontSize: 11, color: 'var(--ink-soft)' }}>Tap to open payment app directly</div>
        </div>

        {/* Transaction ID */}
        <div style={{ background: 'white', borderRadius: 16, padding: '20px 22px', border: '1px solid var(--border)', marginBottom: 16 }}>
          <label style={{ display: 'block', fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--ink-mid)', marginBottom: 8 }}>
        ...
        <button onClick={handlePlace} disabled={placing}
          style={{ width: '100%', padding: '16px', background: placing ? '#666' : 'var(--forest-deep)', color: 'white', border: 'none', borderRadius: 14, fontSize: 16, fontWeight: 700, cursor: placing ? 'not-allowed' : 'pointer', fontFamily: 'var(--font-body)', marginBottom: 12 }}>
          {placing ? '⏳ Placing Order…' : '✅ I Have Paid — Confirm Order'}
        </button>

        <button onClick={() => setStep('details')}
          style={{ width: '100%', padding: '12px', background: 'transparent', color: 'var(--ink-soft)', border: '1.5px solid var(--border)', borderRadius: 14, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
          ← Back to Details
        </button>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
              <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(20px,3vw,32px)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {[
                    ...
                    { k: 'phone',   l: 'Phone Number *',    ph: '10-digit number',    t: 'tel'   },
                    { k: 'email',   l: 'Email (optional)',  ph: 'your@email.com',     t: 'email' },
                    { k: 'address', l: 'Delivery Address',  ph: 'House, street, area',t: 'text'  },
              ...
              <div style={{ background: 'white', borderRadius: 20, padding: 'clamp(20px,3vw,32px)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  {[
                    { k: 'upi', label: '📱 UPI / QR Code', desc: 'GPay, PhonePe, Paytm' },
                    { k: 'cod', label: '💵 Cash on Delivery', desc: 'Pay when delivered' },
                  ].map(m => (
                    <label key={m.k} onClick={() => set('paymentMethod', m.k)} style={{
                      ...
                      <input type="radio" name="payment" value={m.k} checked={form.paymentMethod === m.k} onChange={() => set('paymentMethod', m.k)} style={{ accentColor: 'var(--forest)', marginBottom: 6 }} />
                      <div style={{ fontWeight: 700, fontSize: 14, color: form.paymentMethod === m.k ? 'var(--forest-deep)' : 'var(--ink)' }}>{m.label}</div>
                      <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginTop: 2 }}>{m.desc}</div>
                    </label>
                  ))}
                </div>
...
>
  🚚 Delivery available only in Raipur, Chhattisgarh (CG)
                  ...
                  <button type="submit"
                    style={{ width: '100%', padding: '16px', background: 'var(--forest-deep)', color: 'white', border: 'none', borderRadius: 14, fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--font-body)' }}>
                    {form.paymentMethod === 'cod' ? '✅ Place Order (COD)' : '📱 Proceed to Pay →'}
                  </button>
                  <div style={{ textAlign: 'center', marginTop: 12, fontSize: 12, color: 'var(--ink-soft)' }}>
