...
import { STATIC_PRODUCTS, getProducts } from '../services/api';
...
const CATS = [
  ...
  { key: 'mango', label: 'Mangoes', Icon: (props) => <MangoIcon width={14} height={14} style={{ fill: 'currentColor', display: 'block' }} {...props} /> },
        ...
        <div className="container" style={{ padding: 'clamp(10px,1.5vw,16px) 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ position: 'relative', flexShrink: 0 }}>
            ...
            <input type="search" placeholder="Search products…" value={search} onChange={e => setSearch(e.target.value)}
              style={{ paddingLeft: 36, paddingRight: 16, paddingTop: 9, paddingBottom: 9, border: '1.5px solid var(--border)', borderRadius: 100, fontSize: 13, fontFamily: 'var(--font-body)', outline: 'none', background: 'var(--cream)', color: 'var(--ink)', minWidth: 200, transition: 'border-color 0.2s' }}
        ...
        <div className="container">
          ...
          <AnimatePresence mode="wait">
            {filtered.length === 0 ? (
              ...
              <motion.div key={`${cat}-${search}`} className="grid-auto" variants={stagger} initial="hidden" animate="visible">
